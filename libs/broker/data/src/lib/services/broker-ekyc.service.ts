...
export class BrokerEkycService extends HttpBaseService {
  ...
  ): Observable<ApiResponse<PresignUploadResponse>> {
    return this.post<ApiResponse<PresignUploadResponse>>(
      `${this.BASE_PATH}/submissions/presign`,
      request
    );
  }

  submitEkyc(submission: EkycSubmission): Observable<
    ApiResponse<{
      submissionId: number;
      status: string;
      message: string;
  ...
  getFileDownloadUrl(fileId: string): Observable<
    ApiResponse<{
      downloadUrl: string;
      expiresIn: number;
    }>
  ...
  getSubmission(assignmentId: number): Observable<
    ApiResponse<{
      assignmentId: number;
      documents: Array<{
        id: number;
