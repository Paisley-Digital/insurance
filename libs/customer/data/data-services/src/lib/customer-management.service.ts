...
export class CustomerManagementService extends HttpBaseService {
  ...
  getDashboard(): Observable<ApiResponse<CustomerDashboard>> {
    return this.get<ApiResponse<CustomerDashboard>>(`${this.BASE_PATH}/dashboard`);
  }
  ...
  getEmployer(employerId: number): Observable<ApiResponse<Employer>> {
    return this.get<ApiResponse<Employer>>(`${this.BASE_PATH}/employers/${employerId}`);
  }

  getEmployerEmployees(employerId: number): Observable<ApiResponse<Employee[]>> {
    return this.get<ApiResponse<Employee[]>>(`${this.BASE_PATH}/employers/${employerId}/employees`);
  }

  getEmployee(employeeId: number): Observable<ApiResponse<Employee>> {
    return this.get<ApiResponse<Employee>>(`${this.BASE_PATH}/employees/${employeeId}`);
  }

  getEmployerKycStatus(employerId: number): Observable<ApiResponse<KycStatus>> {
    return this.get<ApiResponse<KycStatus>>(`${this.BASE_PATH}/employers/${employerId}/kyc-status`);
  }
}
